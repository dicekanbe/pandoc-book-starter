...
% jlreq の章設定：デフォルトで番号付き
\ModifyHeading{chapter}{
  label_format={第\thechapter 章\hspace{1em}}
}

% 番号なし章（\chapter*）のラベルを空にする
\makeatletter
\let\jlreq@original@schapter\@schapter
\renewcommand{\@schapter}[1]{%
  % 一時的に label_format を空に
  \ModifyHeading{chapter}{label_format={}}%
  \jlreq@original@schapter{#1}%
  % 元に戻す
  \ModifyHeading{chapter}{label_format={第\thechapter 章\hspace{1em}}}%
}
\makeatother
